...
| | `ws` | List/filter/sort workspaces, group by org/project, filter by pending runs, fetch subresources (current-run, current-state-version, current-configuration-version, current-assessment-result), show run history with phase durations (`--runs`) |
